...
public class TituloContaPagarDAO {
    ...
    public void inserir(TituloContaPagarModel t) throws SQLException {
        String sql = "INSERT INTO titulos_contas_pagar("
                + "id, fornecedor_id, plano_conta_id, pedido_id, codigo_selecao, data_geracao, valor_total, status, observacoes"
                + ") VALUES (?,?,?,?,?,?,?,?,?)";
        ...
        try (Connection c = DB.get();
            ...
            ps.setString(4, t.getPedidoId());
            ps.setString(5, t.getCodigoSelecao());
            ps.setString(6, t.getDataGeracao());
            ps.setDouble(7, t.getValorTotal());
            ps.setString(8, t.getStatus());
            ps.setString(9, t.getObservacoes());
            ps.executeUpdate();
        }
    }
    ...
    public void atualizar(TituloContaPagarModel t) throws SQLException {
        String sql = "UPDATE titulos_contas_pagar SET "
                + "fornecedor_id=?, plano_conta_id=?, pedido_id=?, codigo_selecao=?, data_geracao=?, valor_total=?, status=?, observacoes=? "
                + "WHERE id=?";
